...
public sealed class ExchangeRepository : GenericRepository<Exchange>, IExchangeRepository
{
	...
	public async Task<IReadOnlyDictionary<string, Exchange>> GetExistingExchangesAsync(IEnumerable<string> nameIds)
	{
		return await _dbContext.Exchanges
			.Where(c => nameIds.Contains(c.NameId))
			.ToDictionaryAsync(c => c.NameId);
	}
