...
public sealed class ExchangeService : IExchangeService
{
	...
	{
		var names = exchanges.Select(r => r.NameId);
		var existingExchanges = await _exchangeRepository.GetExistingExchangesAsync(names);
		...
		{
			if (existingExchanges.TryGetValue(exchange.NameId, out var existingExchange))
			{
				existingExchange.UpdateMarketData(
					Money.Create(exchange.Volume.Amount, exchange.Volume.Currency),
					exchange.Country,
					exchange.Url,
		...
		_logger.LogInformation("Inserted exchange names: {InsertedExchangeNames}", insertedExchangeNames);
		_logger.LogInformation("Updated exchange names: {UpdatedExchangeNames}", updatedExchangeNames);
	}
}